...
export default function AddScreen() {
  ...
  useEffect(() => {
    trackEvent('page_view', { page: 'add_expense_mobile' });
    // In Expo, EXPO_PUBLIC_ prefix is needed for env vars
  ...
  const handleScanReceipt = () => {
    ...
    trackEvent('ocr_scan_started_mobile');
    ...
    setTimeout(() => {
      try {
        ...
        trackEvent('ocr_scan_success_mobile', { merchant: result.merchant, total: result.total });
        Alert.alert(t('common.success'), t('addExpense.scanningComplete', 'Receipt scanned successfully.'));
      } catch (error) {
        setOcrStatus('error');
        reportError(error instanceof Error ? error : new Error(String(error)), { context: 'ocr_scan_mobile' });
        trackEvent('ocr_scan_failed_mobile');
      }
    }, 2000);
  };
  ...
  const handleCreateExpense = async () => {
    ...
    const payload: CreateSplitDto = {
      ...
      currency: 'USD',
      participants: selectedSplits.map(s => ({
        userId: s.userId,
        name: s.name,
        amount: s.amount,
      })),
    ...
    trackEvent('expense_creation_started_mobile', { amount: payload.amount, currency: payload.currency });
